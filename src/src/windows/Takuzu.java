...
public class Takuzu {
	...
	public void genCase(int row, int column) {
		...
		if (this.isCellEmpty(row, column)) {
			...
			if (this.isDoubleLine(row, column)) {
				...
				if (this.countLine(row, UN) == T/2) {
					this.setValueString(row, column, ZERO);
				} else if (this.countLine(row, ZERO) == T/2) {
	...
	public void randomAffichage(Takuzu tak) {
		this.resetGrille();
	...
	public boolean compGrille(Takuzu tak) {
		for (int row = 0 ; row < T ; row++) {
			for (int col = 0 ; col < T; col++) {
	...
	public void genGrilleFromFile() {
	    ...
	    int random = getRNIR(1,22685);

	    try {
	      //lire le fichier file.txt
	      FileReader file = new FileReader("./src/Takuzu_lib/takliste.txt");
